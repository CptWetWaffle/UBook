...
namespace UBook.FrontendViews
{
    ...
    /// </summary>
    public sealed partial class RestaurantView: Page, IObserver
    {
        private IUser _user;
        ...
        public RestaurantView()
        {
            ...
            foreach (var service in data.Services.Where(s => s.Type.Name.ToLower() == "restaurant"))
            {
        ...
        private void AppBarButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            foreach (var h in data.Services.Where(h => h.Type.Name.ToLower() == "restaurant"))
            {
                if (!h.ServiceValues.Any(namai => namai.Value.Contains(txtRest.Text))) continue;
                var g = new GridViewItem
                {
                    Name = "" + h.Id,
                    Content = h.ToString(),
        ...
        private void btnRem_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                var m = new MessageDialog("Successfully removed a restaurant!", "Success!");
                m.ShowAsync();
            }
            catch (Exception)
            {
                var m = new MessageDialog("There was an error trying to remove that Error", "Success!");
                m.ShowAsync();
            }
